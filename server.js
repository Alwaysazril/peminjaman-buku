...
app.post('/tambah', (req, res) => {
    const d = req.body;
    if (!fs.existsSync('data_peminjaman.txt')) {
        const h = "PEMINJAM       | JUDUL BUKU           | NO | ID | PENERBIT\n" +
                  "----------------------------------------------------------\n";
        fs.writeFileSync('data_peminjaman.txt', h);
    }
    const baris = (d.namaPeminjam || '').padEnd(14) + " | " + (d.judulBuku || '').padEnd(20) + " | " + (d.nomorBuku || '').padEnd(2) + " | " + (d.idBuku || '').padEnd(2) + " | " + (d.penerbit || '') + "\n";
